...
export default async function DashboardOverview() {
  ...
  return <div className="mx-auto max-w-7xl space-y-6 p-6"><div className="flex flex-wrap items-center justify-between gap-4"><div><h1 className="text-3xl font-bold tracking-tight">Mission Control</h1><p className="text-muted-foreground">Your tenant workspace and BRIXTA service availability.</p></div><div className="flex gap-2"><Button render={<Link href="/sources" />}>Add source</Button><Button variant="outline" render={<Link href="/ingestion" />}>Run pipeline</Button></div></div><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3"><ServiceStatusCard name="BRIXTA Core" description="FastAPI control plane" healthy={state(core)} detail={core.error} />{principal.is_admin && <><ServiceStatusCard name="Celery" description="Pipeline workers" healthy={state(celery)} detail={celery.error} /><ServiceStatusCard name="Redis" description="Broker and queue state" healthy={state(redis)} detail={redis.error} /><ServiceStatusCard name="Artifact storage" description="Local or MinIO backend" healthy={state(storage)} detail={storage.provider} /><ServiceStatusCard name="Docker" description="Local container engine" healthy={state(docker)} detail={docker.error} /></>}</div></div>;
}
