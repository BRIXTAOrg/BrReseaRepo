...
export default async function RedisPage() {
  const [health, queues, docker] = await Promise.all([fetchPythonApiServer("/prod/redis"), fetchPythonApiServer("/prod/redis/queues"), fetchPythonApiServer("/prod/docker/containers")]) as [{ healthy?: boolean; error?: string }, { queues?: Queue[] }, { containers?: Container[] }];
